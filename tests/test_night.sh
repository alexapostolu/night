...
night_files=($(find programs/ccc programs/cs50 programs/math -type f -name "*.night" | sort))

# Standard input and output files corresponding to the Night source code files
inputs=($(find stdio/ccc stdio/cs50 stdio/math -type f -name "*_input.txt" | sort))
expected_outputs=($(find stdio/ccc stdio/cs50 stdio/math -type f -name "*_expected.txt" | sort))
...
for i in "${!night_files[@]}"; do
    ...
    if [ $exit_code -ne 0 ]; then
        echo -e "\n\e[31mFailed. \e[33mCompile error with exit code $exit_code.\e[0m\n"
        cat "$actual_output"
        ((tests_failed++))
        continue
    fi
