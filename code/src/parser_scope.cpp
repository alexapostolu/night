	...
	Location const& loc)
{
	...
	if (vars.contains(name))
		night::create_minor_error("variable '" + name + "' is already defined", loc);

	if (var_id == bytecode_t_lim)
		night::create_minor_error("only " + std::to_string(bytecode_t_lim) + " variables allowed per scope", loc);
