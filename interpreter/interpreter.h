...
void Interpreter(night::array<night::array<Token> >& code)
{
	...
	for (int a = 0; a < code.length(); ++a)
	{
		...
		else if (code[a].length() >= 4 && IsAssignExpr(code[a][1]))
		{
			...
			if (variable != nullptr)
			{
				Token evalExpr = ParseExpression(code[a].access(2, code[a].length() - 1), CONTAINERS);
			...
			for (int b = 3, elementIndex = 0, bracketCount = 0; b < code[a].length() - 2; ++b)
			{
					...
					code[a][b + 2].type == TokenType::SEMICOLON)
				{
					...
					elementExpression = night::array<Token>();
				}
			}
		}
		...
		{
			int bracketIndex = 4;
			for (; code[a][bracketIndex].type != TokenType::OPEN_CURLY; ++bracketIndex);
			bracketIndex--;
		...
		else if (code[a].length() == 6 && IsType(code[a][1]) && IsOpenSquare(code[a][1]))
		{
		...
		else if (code[a].length() >= 8 && IsType(code[a][1]) && IsOpenSquare(code[a][1]))
		{
			if (code[a][code[a].length() - 2].type == TokenType::VARIABLE)
			{
				if (GetObject(variables, code[a][2]) != nullptr)
					throw Error(night::_invalid_array_, code[a], 2, 2, "arrays cannot be assigned to values of type 'int'; arrays can only be assigned to other arrays");

				Array* assignArray = GetObject(arrays, code[a][2]);
				if (GetObject(arrays, code[a][2]) == nullptr)
					throw Error(night::_invalid_array_, code[a], 2, 2, "array has not been defined");
			...
			int closeSquare = 4;
			for (; code[a][closeSquare].type != TokenType::ASSIGNMENT; ++closeSquare);
			closeSquare -= 2;
			...
			for (int b = closeSquare + 4; code[a][b + 1].type != TokenType::SEMICOLON; ++b)
			{
					...
					code[a][b + 2].type == TokenType::SEMICOLON)
				{
					if (elementIndex == arrayLength)
						throw Error(night::_invalid_array_, code[a], 0, code[a].length() - 1, "too many elements in array; expected "_s + night::itos(arrayLength) + " elements");
			...
			{
				for (int b = elementIndex; b < arrays.back().elems.length(); ++b)
				{
					arrays.back().elems[b] = Token{
						night::ttov(code[a][0].type),
						DefaultValue(code[a][0].type)
					};
				}
